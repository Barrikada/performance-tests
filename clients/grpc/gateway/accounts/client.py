...
class AccountsGatewayGRPCClient(GRPCClient):
    ...
    def __init__(self, channel: Channel) -> None:
        ...
        self.stub: AccountsGatewayServiceStub = AccountsGatewayServiceStub(channel)
    ...
    def get_accounts(self, user_id: str) -> GetAccountsResponse:
        """
        Список счетов пользователя по user_id.

        :param user_id: Идентификатор пользователя.
        :return: Ответ GetAccountsResponse.
        """
        request: GetAccountsRequest = GetAccountsRequest(user_id=user_id)
        return self.get_accounts_api(request)

    def open_deposit_account(self, user_id: str) -> OpenDepositAccountResponse:
        """
        Открыть депозитный счёт для пользователя.

        :param user_id: Идентификатор пользователя.
        :return: Ответ OpenDepositAccountResponse.
        """
        request: OpenDepositAccountRequest = OpenDepositAccountRequest(user_id=user_id)
        return self.open_deposit_account_api(request)

    def open_savings_account(self, user_id: str) -> OpenSavingsAccountResponse:
        """
        Открыть сберегательный счёт для пользователя.

        :param user_id: Идентификатор пользователя.
        :return: Ответ OpenSavingsAccountResponse.
        """
        request: OpenSavingsAccountRequest = OpenSavingsAccountRequest(user_id=user_id)
        return self.open_savings_account_api(request)

    def open_debit_card_account(self, user_id: str) -> OpenDebitCardAccountResponse:
        """
        Открыть дебетовый счёт (с картами) для пользователя.

        :param user_id: Идентификатор пользователя.
        :return: Ответ OpenDebitCardAccountResponse.
        """
        request: OpenDebitCardAccountRequest = OpenDebitCardAccountRequest(user_id=user_id)
    ...
    def open_credit_card_account(self, user_id: str) -> OpenCreditCardAccountResponse:
        """
        Открыть кредитный счёт для пользователя.

        :param user_id: Идентификатор пользователя.
        :return: Ответ OpenCreditCardAccountResponse.
        """
        request: OpenCreditCardAccountRequest = OpenCreditCardAccountRequest(user_id=user_id)
