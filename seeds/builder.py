...
def _make_top_up_operation(operations_client, card_id: str, account_id: str):
    """
    gRPC-клиент называет метод make_top_up_operation, HTTP — make_top_up_operations.
    """
    maker = getattr(operations_client, "make_top_up_operation", None)
    if callable(maker):
        return maker(card_id=card_id, account_id=account_id)
    maker = getattr(operations_client, "make_top_up_operations", None)
    if callable(maker):
        return maker(card_id=card_id, account_id=account_id)
    raise AttributeError(
        "operations_gateway_client must expose make_top_up_operation or make_top_up_operations"
    )
...
class SeedsBuilder:
    ...
    def build_top_up_operation_result(self, card_id: str, account_id: str) -> SeedOperationResult:
        ...
        """
        response = _make_top_up_operation(
            self.operations_gateway_client,
            card_id=card_id,
            account_id=account_id,
        )
    ...
    def build_debit_card_account_result(self, plan: SeedAccountsPlan, user_id: str) -> SeedAccountResult:
        ...
        - выполняет операции пополнения (top-up), покупки, перевода и снятия наличных
